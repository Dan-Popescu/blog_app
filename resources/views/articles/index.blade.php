                                ...
                                <div class="flex">
                                    ...
                                    <div id="dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                                        ...
                                        @foreach ($categories as $category)
                                            <li class="px-4 py-2 hover:bg-gray-100">
                                                <a href="{{route("articles.category",['category'=>$category])}}" class="inline-flex w-full  py-0.5 px-2 rounded-md  " style="background-color:{{$category->color}}; color:{{isColorDark($category->color)? "black" : "white"}}">{{$category->name}}</a>
                                            </li>
                                        @endforeach
                                        </ul>
                                    </div>
                                    <div class="relative w-full">
                                        <form action="{{route('articles.index')}}" method="GET">
                                            <input type="search" id="search-dropdown" name="titlesearch" value="{{ request()->get('titlesearch') }}" class="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-e-lg border-s-gray-50 border-s-2 border border-gray-300 focus:ring-1 focus:ring-blue-500 focus:border-blue-500" placeholder={{__('Search articles')}} required />
